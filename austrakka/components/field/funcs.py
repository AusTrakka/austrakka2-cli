...
@logger_wraps()
def add_field(name: str, typename: str, can_visualise: str, column_order: int, show_at_start: bool):
    """
    ...
    if can_visualise=='viz':
        if typename in ["date", "number", "string"]:
            logger.warning(f"Setting colour-nodes flag on field {name} of type {typename}. This may work poorly as colour visualisations are configured for a small discrete set of values.")
        can_visualise = True
        ...
        if typename=="string":
            logger.warning(f"Setting default of --no-colour-nodes on field {name} due to type {typename}. If this string field should be allowed to be used for colour visualisations, set --colour-nodes.")
...
@logger_wraps()
def update_field(name: str, new_name:str, typename: str, can_visualise: str, column_order: int, set_show: str):
    """
