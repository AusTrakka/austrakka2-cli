...
def download_fastq_for_each_sample(
    ...
    for ssi in samples_seq_info:
        ...
        for seq_dto in ssi[1]:
            ...
            if not sample_name:
                logger.error(
                    f'Encountered sample name. Skipping all sequences associated with the sample...')
                continue

            if dto_read not in (ONE, TWO) or not seq_type:
                logger.error(
                    f'Error in sample: {sample_name}. Found sequence with invalid Read or Type. Skipping...')
                continue
