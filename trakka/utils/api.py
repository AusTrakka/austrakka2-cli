...
from trakka.utils.exceptions import UnknownResponseException
from trakka.utils.exceptions import UnauthorizedException
...
from trakka import __version__
...
def _get_default_headers(
    ...
    default_headers = {
        ...
        'Authorization': f'Bearer {TrakkaCxt.get_value(CxtKey.TOKEN)}',
        'User-Agent': f'trakka/{__version__}',
        'X-Client-Session-ID': TrakkaCxt.get_value(CxtKey.SESSION_ID),
    }
...
def _get_url(path: str):
    return f'{TrakkaCxt.get_value(CxtKey.URI)}/api/{path}'
...
def _get_client(
    ...
    return httpx.Client(
        ...
        timeout=None,
        http2=TrakkaCxt.get_value(CxtKey.USE_HTTP2),
    )
...
@_use_http_client(parse_resp=False)
def api_get_stream(
        path: str,
    ...
    if params is None:
        params = {}
    resp: httpx.Response
    with client.stream("GET", _get_url(path), params=params) as resp:
        try:
            resp.raise_for_status()
        except HTTPStatusError:
            resp.read()
            get_response(resp, log_resp=True)
        func(resp)
