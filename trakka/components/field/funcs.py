...
from trakka.utils.output import print_dataframe, get_viewtype_columns
from trakka.utils.paths import METADATA_COLUMN_V2_PATH
...
def list_fields(view_type: str, out_format: str):
    ...
    """
    response = api_get(
        path=f"{METADATA_COLUMN_V2_PATH}",
    )
...
def update_field(
    ...
    api_patch(
        path=f"{METADATA_COLUMN_V2_PATH}/{name}",
        data=patch_fields
    )
...
def list_field_proformas(name: str, out_format: str):
    ...
    result = api_get(
        path=f"{METADATA_COLUMN_V2_PATH}/{name}/proformas"
    )
...
def disable_field(name: str):
    ...
    """
    api_patch(
        path=f"{METADATA_COLUMN_V2_PATH}/{name}/disable"
    )
...
def enable_field(name: str):
    ...
    """
    api_patch(
        path=f"{METADATA_COLUMN_V2_PATH}/{name}/enable"
    )
